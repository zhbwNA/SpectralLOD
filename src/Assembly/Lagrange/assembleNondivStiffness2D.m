...
for q = 1:nQuad
    ...
    [d11, d12, d21, d22, beta1, beta2] = evalNondivCoefficients(coef, xq, yq);

    Dq = squeeze(DphiRef(q, :, :));
    Dx = g1x * Dq(:,1).' + g2x * Dq(:,2).' + g3x * Dq(:,3).';
    Dy = g1y * Dq(:,1).' + g2y * Dq(:,2).' + g3y * Dq(:,3).';

    diffPart = (d11 .* Dx(:,bb) + d12 .* Dy(:,bb)) .* Dx(:,aa) + ...
               (d21 .* Dx(:,bb) + d22 .* Dy(:,bb)) .* Dy(:,aa);
    driftPart = phi(q, aa) .* (beta1 .* Dx(:,bb) + beta2 .* Dy(:,bb));
    S = S + (2 * weight(q) * area) .* (diffPart + driftPart);
end
...
function [d11, d12, d21, d22, beta1, beta2] = evalNondivCoefficients(coef, x, y)
if isnumeric(coef) || isa(coef, 'function_handle')
    c = evalPDECoefficient(coef, x, y, [], []);
    d11 = c; d22 = c;
    d12 = zeros(size(x)); d21 = zeros(size(x));
    beta1 = zeros(size(x)); beta2 = zeros(size(x));
    return;
end
...
beta1 = evalField(coef, 'beta1', x, y, 0);
beta2 = evalField(coef, 'beta2', x, y, 0);
end
